# -*- coding: utf-8 -*-
# tente algo como
def index(): return dict(message="hello from acs_mercadoria.py")
def conferir():
  ...
  redirect(URL('acessar_carrada', args=item_carrada.carrada))
  return locals()
...
def conferir_retorno():
  ...
  item_carrada.update_record()
  redirect(URL('retorno', args=item_carrada.projeto))
  return locals()
def carradas():
...
def alterar_retorno():
    ...
    item_carrada = db.item_carrada(request.args(0, cast=int))
    carrada = db.carrada(item_carrada.carrada)
    if item_carrada.descricao.startswith('rtnc'):
        redirect(URL('acessar_carrada', args=carrada.id))
    projeto = db.projeto(carrada.projeto)
    if projeto.venda_finalizada:
        session.flash = 'A venda já foi finalizada'
