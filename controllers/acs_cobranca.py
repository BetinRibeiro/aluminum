...
def relacao_cobradores():
    ...
    return locals()
@auth.requires_login()
def inserir_cobrador():
    response.view = 'generic.html' # use a generic view
    sub_venda = db.sub_venda(request.args(0, auth.user))
    db.cobrador.sub_venda.default = sub_venda.id
    db.cobrador.sub_venda.writable = False
    form = SQLFORM(db.cobrador).process()
    if form.accepted:
        response.flash = 'Formulario aceito'
        redirect(URL('relacao_cobradores', args=[sub_venda.id]))
    elif form.errors:
...
def alterar_pessoa():
    response.view = 'generic.html' # use a generic view
    cobrador = db.cobrador(request.args(0, auth.user))
    db.cobrador.sub_venda.writable = False
    db.cobrador.id.readable = False
    form = SQLFORM(db.cobrador, request.args(0, cast=int), deletable=True)
    if form.process().accepted:
        session.flash = 'Atualizado'
        redirect(URL('relatorio_cobranca', args=[cobrador.sub_venda]))
    elif form.errors:
